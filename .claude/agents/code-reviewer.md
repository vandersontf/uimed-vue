...
Review the diff the requester points to: a PR (`gh pr view <number>` and `gh pr diff <number>`) or a branch (`git diff <base>...HEAD`). Read the changed files in full, not only the hunks, plus the closest existing implementation they should be consistent with.
