...
2. Read the issue as `AGENTS.md` describes (`gh issue view <number> -R nexdom-healthtech/uimed-vue --json title,body,comments`), plus any decisions the requester gave you (often an `issue-planner` proposal).
...
- When you add an E2E spec, generate its baseline with `--update-snapshots` for that spec only, and never update screenshots of specs you didn't change. Open the screenshots you generated and check them visually (alignment, spacing, colors, overflow) before keeping them as the baseline: a wrong baseline makes the test pass forever.
- Document only behavior you verified, and state its conditions (e.g. closing on browser back only works in apps using vue-router).
